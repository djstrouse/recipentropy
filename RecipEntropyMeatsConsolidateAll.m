function [] = RecipEntropyMeatsConsolidateAll(Ningred_used,consolidate_orders,Nsubsamps)
% Consolidates maxent data for Ningred_used most common meats
%
...
if consolidate_orders
  orders = 0:2;
  RecipEntropyMeatsConsolidate(Ningred_used,orders,Nsubsamps);
end
...
load(sprintf('data/meats/%iingred/order%i/large.mat',Ningred_used,0));
load(sprintf('data/meats/%iingred/order%i/small.mat',Ningred_used,0));
load(sprintf('data/meats/%iingred/order%i/large.mat',Ningred_used,1));
load(sprintf('data/meats/%iingred/order%i/small.mat',Ningred_used,1));
maxent1_small = maxent_small; clear maxent_small;
maxent1_large = maxent_large; clear maxent_large;
load(sprintf('data/meats/%iingred/order%i/large.mat',Ningred_used,2));
load(sprintf('data/meats/%iingred/order%i/small.mat',Ningred_used,2));
...
% save
save(...
  sprintf('data/meats/%iingred/large.mat',Ningred_used),...
  'maxent1_large',...
  'maxent2_large',...
  'freq_large',...
  'maxent1v2',...
  ...
  '-v7.3');
save(...
  sprintf('data/meats/%iingred/small.mat',Ningred_used),...
  'maxent1_small',...
  'maxent2_small',...
  'freq_small',...
  'maxent1v2',...
